...
A Django-based microservices bookstore system with 12 independent services communicating via HTTP through an API Gateway. The system uses PostgreSQL with separate databases per service for data isolation.
...
- `cart-service` → `book-service`: Validates book availability
- `recommender-ai-service` → `book-service` + `comment-rate-service`: Aggregates data for recommendations
...
- `customer_db`, `book_db`, `cart_db`, `staff_db`, `manager_db`, `catalog_db`, `order_db`, `ship_db`, `pay_db`, `comment_rate_db`, `recommender_db`, `gateway_db`
